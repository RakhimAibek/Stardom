...
class TripTableViewCell: UITableViewCell {
    ...
    let chooseHeightLabel: UILabel = {
        let label = UILabel()
        label.text = "Готов(а) к командировкам"
    ...
    private func setupViews() {
        [chooseHeightLabel, decisionSwitch].forEach {
            self.addSubview($0)
        }
        
        chooseHeightLabel.anchor(top: nil, leading: leadingAnchor, bottom: nil, trailing: nil, padding: .init(top: 0, left: 16, bottom: 0, right: 0), size: .init(width: 169, height: 17))
        chooseHeightLabel.centerYAnchor.constraint(equalTo: self.centerYAnchor).isActive = true
